...
export default function PriorityMatrix({ rankings, setRankings }) {
  ...
  return (
    <div className="bg-charcoal border border-parchment/10 p-8 md:p-12 rounded-lg contact-reveal">
      <div className="mb-10 flex items-center justify-between text-xs tracking-widest text-gold uppercase border-b border-parchment/10 pb-4">
        <span>Part 2: Priority Matrix</span>
      </div>

      <div className="space-y-4 mb-8">
        <h3 className="font-serif text-3xl md:text-4xl text-parchment">Rank Your Constraints</h3>
        <p className="text-parchment/60 font-light max-w-2xl">
          ...
          {rankings.map((priority, index) => (
            <div key={priority.id} className="flex items-center gap-4 p-4 bg-parchment/5 rounded border border-parchment/10">
              ...
              <div className="flex flex-col gap-1">
                <button onClick={(e) => { e.preventDefault(); moveUp(index); }} disabled={index === 0} className="p-1 hover:text-gold disabled:opacity-20 transition-colors">↑</button>
                <button onClick={(e) => { e.preventDefault(); moveDown(index); }} disabled={index === rankings.length - 1} className="p-1 hover:text-gold disabled:opacity-20 transition-colors">↓</button>
              </div>
              <button onClick={(e) => { e.preventDefault(); togglePriority(priority); }} className="p-2 ml-4 hover:text-red-400 text-parchment/40 transition-colors">✕</button>
            </div>
          ))}
        </div>
      )}
